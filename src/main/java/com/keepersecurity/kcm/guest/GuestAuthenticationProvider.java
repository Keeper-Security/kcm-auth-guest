...
public class GuestAuthenticationProvider extends AbstractAuthenticationProvider {
    ...
    /**
     * Set of all usernames generated by {@link #getGuestUsername()} that are
     * still in active use. Any generated usernames that are no longer in
     * active use must be manually removed.
     */
    private static final Set<String> activeUsers = Collections.newSetFromMap(new ConcurrentHashMap<>());
    ...
    /**
     * Returns a uniquely-generated username by appending a random suffix to the
     * username prefix "guest-". The random suffix contains at least 64 bits of
     * entropy. To avoid leaking memory, all values returned by this function
     * must eventually be manually removed from the {@link #activeUsers} set.
     *
     * NOTE: It is extremely unlikely that any two generated usernames will
     * collide, but we track and regenerate usernames based on active values
     * here to ensure this cannot happen.
     *
     * @return
     *     A uniquely-generated username.
     */
    private String getGuestUsername() {
        ...
        do {
            ...
            username = "guest-" + BaseEncoding.base64Url().encode(bytes);

        } while (!activeUsers.add(username));
    ...
    public AuthenticatedUser authenticateUser(final Credentials credentials)
        ...
        if (request != null && isGuestUser(request)) {
            ...
            // session
            final String identifier = getGuestUsername();
